...
```{r}
fit <- bcfa(model, data = semd, bcontrol = list(cores = 3), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)

saveRDS(fit, "../saves/bcfa_prag")

fit2 <- bcfa(model2, data = semd, bcontrol = list(cores = 3), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)

saveRDS(fit2, "../saves/bcfa_single")

fit3 <- bcfa(model3, data = semd, bcontrol = list(cores = 3), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)

saveRDS(fit3, "../saves/bcfa_separate")
```
...
```{r}
#model weights
w1 <- exp(-0.5*0)/(exp(-0.5*0)+exp(-0.5*6.7)+exp(-0.5*7.1)+exp(-0.5*8.9))
```
