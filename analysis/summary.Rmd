...
```{r}
# fit <- bcfa(model, data = semd, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)
# saveRDS(fit, "../saves/bcfa_prag.rds")
fit <- readRDS("../saves/bcfa_prag.rds")

# fit2 <- bcfa(model2, data = semd, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)
# saveRDS(fit2, "../saves/bcfa_single.rds")
fit2 <- readRDS("../saves/bcfa_single.rds")

# fit3 <- bcfa(model3, data = semd, bcontrol = list(cores = 4), target = "stan", n.chains = 4, burnin = 5000, sample = 5000)
...
library(parameters)
library(effectsize)
model_parameters(fit, standardize = T)
```
...
corrsa <- cor.test(prsa%>%filter(task == "card_sorting")%>%pull(mode), prsa%>%filter(task == "card_sorting")%>%pull(mean))

ggplot(prsa, aes(x = mean, y = mode))+
