...
task_data <- raw_data%>%
  ...
  mutate(subid = ifelse(subid == "265011", "260934", subid),
         item = ifelse(task == "mutual_exclusivity",
                       ifelse(correct_location == "left", rightObject, leftObject), item))
...
data <- log %>%
  ...
  mutate(check_age = ifelse(subage == substr(age,1,1),T,F),
    ...
    select(id, subage,age,age_month,gender, task,item, trial, correct)
...
data %>%
  ...
  select(id, subage, age, age_month, task, trial, item, correct)%>%
  mutate(c_age_month = age_month - min(age_month),
         c_age = age - min(age))%>%
  write_json("../../../Spin-off/spin-off/data/data_r3.json")
...
ggplot()+
  ...
  geom_pointrange(data = p2, aes(x = as.numeric(as.character(subage))+.5, y = mean, ymin = ci_lower, ymax = ci_upper, col = factor(subage)))+
  facet_wrap(~task)+
