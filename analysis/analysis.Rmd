...
nd1 <- data%>%
  ...
  select(-z_trial,-correct, -task, -test_day, -age_month, -gender, -item, -subage)%>%
  mutate(z_trial = 0)
nd2 <- data%>%
...
pf2 <- ggplot()+
  ...
  geom_jitter(data = p1, aes(x = age, y = mean, pch = study), alpha = .25, width = .05, height = .01)+
  geom_smooth(data = pred, aes(x = age, y = Estimate, ymin = Q2.5, ymax = Q97.5, lty =study), stat = "identity", col = "black", size = 1, alpha = .25)+
...
ggsave(plot = pf2,"../paper/figures/figure2.png", width = 10, height = 4, scale = 1.25)
```
...
blavInspect(fit, what = "postmean")%>%as_tibble(rownames = "index")%>%
  left_join(blavInspect(fit, what = "hpd")%>%as_tibble(rownames = "index"))%>%
  write_csv("../saves/cfa_estimates.csv")
...
```{r}
rsa_model <- readRDS("../../../Spin-off/spin-off/model/output/model_pragBat3.rds")
```
...
prsacor <- ggplot(rsa, aes(x = mean, y = mode))+
  #geom_abline(intercept = 2, slope = 1, lty = 2, alpha = .75)+
  geom_point(pch = 1, alpha = .75)+
  geom_smooth(method = "lm", col= "firebrick")+
  geom_text(data = cor, aes(label = label),y = 4.9, x = 0.2)+
  labs(x = "Mean proportion correct", y = "RSA model estimate (\u03b1)")+
  facet_grid(~task)+
  theme_few()
...
```{r}
rel_rsa_model <- readRDS("../../../Spin-off/spin-off/data/rsa_rel_est.rds")
```
...
ggsave(plot = prsa,"../paper/figures/figure6.png", width = 10, height = 4, scale = 1.25)
```
