...
```{r}
model <- 'prag  =~ " mutual_exclusivity + ad_hoc_implicature + simple_inf
          card  =~ card_sorting
          match =~ match_to_sample'

model2 <- 'fac  =~ mutual_exclusivity + ad_hoc_implicature+ simple_inf + card_sorting + match_to_sample'
...
x <- summary(fit, fit.measures=TRUE, standardized = TRUE, rsquare = T)
