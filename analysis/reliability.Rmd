...
data_r2 <- read_csv("../data/data_r2.csv")%>%mutate(id = paste(id,"r2",sep = "_"))
  ...
  mutate(item = ifelse(task == "mutual_exclusivity" & item == "t22", "carrot", item),
         item = ifelse(task == "mutual_exclusivity" & item == "t37", "duck", item),
         item = ifelse(task == "mutual_exclusivity" & item == "t24", "duck", item))
