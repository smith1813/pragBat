...
# card_sorting_model1 <- brm(correct ~ age + trial + (trial|id),
...
ts <- loo_compare(card_sorting_model1, card_sorting_model2, criterion = "waic")%>%
  as.data.frame() %>%
...
```{r}
p_sum_cs1 <- card_sorting_data %>%
  mutate(subage = as.numeric(as.character(subage)))%>%
  group_by(age,subage,id) %>%
...
samples_cs_1 <- posterior_samples(card_sorting_model1, "^b")%>%
  mutate(sample = 1:length(b_age))

samples_cs_2 <- expand_grid(samples_cs_1,card_sorting_model_data$age)%>%
  mutate(y = plogis(b_Intercept + b_age * `card_sorting_model_data$age`),
         age = `card_sorting_model_data$age` + mean(card_sorting_data$age),
         experiment = "card_sorting")%>%
  filter(sample < 1000)

map_cs <- tibble(
  age = card_sorting_model_data$age,
  int = fixef(card_sorting_model1)[1,1],
  slope = fixef(card_sorting_model1)[2,1]
)%>%
  mutate(y = plogis(int + slope * age),
         age = age + mean(card_sorting_data$age_num))

p_cs <- ggplot() +
  geom_hline(yintercept = .5, lty=2, size = 1)+
  geom_line(data = samples_cs_2, aes(x = age, y = y, group = sample), size = .025, col = "grey")+
  geom_line(data = map_cs, aes(x = age, y = y), size = 1)+
  geom_jitter(data = p_sum_cs1,aes(x = age, y= correct), width = .00, height = .01, alpha = .5)+
  geom_pointrange(data = p_sum_cs2, aes(x = subage+.5, y = mean, ymin = ci_lower, ymax = ci_upper),size = .8, position = position_dodge(width = .2), pch = 4)+
  labs(x="Age",y="Proportion correct")+
  theme_few() +
  ylim(-0.05,1.05)+
  xlim(3,5)+
