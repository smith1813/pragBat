...
f2 <- 
  brm(data = reli, 
      ...
      prior = c(prior(gamma(2, .1), class = nu),
                ...
                prior(normal(0, 100), class = sigma, resp = Day1),
                prior(normal(0, 100), class = sigma, resp = Day2),
...
xx <- bayes.cor.test( ~Day1 + Day2, data= card_sorting_data, method = "spearman")

$stats%>%
  as_data_frame(.,rownames = "par")%>%
  filter(par == "rho")%>%
...
x <- bayes.cor.test(reli$Day1, reli$Day2)
...
as_data_frame(y,rownames = "par")%>%filter(par == "rho")%>%pull(mean)
