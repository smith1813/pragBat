...
files <- dir("~/Work/Local/PragBat/raw_data/")

raw_data <- data_frame()
for (f in files) {
  jf <- paste("~/Work/Local/PragBat/raw_data/",f,sep="")
...
log <- read_ods("../../VP Liste_Manuel.ods", sheet = 2)%>%
  mutate(age = as.numeric(difftime(test_day_1, `birthday`, units = "days"))/365.25,
         delay = as.numeric(difftime(test_day_2,test_day_1, units = "days")),
         id = paste("Vp",`ID`, sep = ""))%>%
...
write_csv(data, "../data/data_r1.csv")
